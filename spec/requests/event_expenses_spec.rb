...
RSpec.describe ExpensesController, type: :request do
  ...
  let(:event) { create(:event, trip_id: trip.id) }

  describe "For Event" do
    context "Authorization" do
      let(:path) { trip_event_expenses_path(trip.id, event.id) }
    ...
    context "#create" do
      ...
      it "creates a expense" do
        expense = build(:expense, event_id: event.id)
        ...
        expect(json[:event_id]).to eq expense.event_id
      end
    ...
    context "#update" do
      ...
      let(:path) { trip_event_expense_path(trip.id, event.id,  expense.id) }
    ...
    context "#destroy" do
      ...
      it "raises error" do
        path = trip_event_expense_path(trip.id, event.id, "id-does-not-exist")
        delete path, headers: headers
        response_matches?(error_response)
